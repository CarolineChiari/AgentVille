 ...
 *
 * `windowId=_blank` because without it a folder no window has yet replaces the folder in the last
 * active window (the editor's default for a link; Windows hits it most, where a link never comes
 * from the dock), which stops whatever agent was running there. With it, the editor still looks
 * for a window already on the folder first and only opens a new one when there is none. An editor
 * too old to know the parameter ignores the query and behaves as before.
 */
