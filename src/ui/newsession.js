...
export function createNewSession(root, village, { onRemember = () => {} } = {}) {
  ...
  /** Say what will happen. Only the terminal can take a model, so picking one moves you there. */
  function explain(changed) {
    const t = box.querySelector('[data-f="target"]')
    const m = box.querySelector('[data-f="model"]')
    if (changed === 'model' && m.value && t.value !== 'terminal') t.value = 'terminal'
    const note = {
      vscode: 'Opens in VS Code with the prompt typed in — press Enter there to send it. VS Code uses your default model; pick another in its model menu, or choose Terminal here.',
      terminal: `Opens a terminal in the folder running Claude Code${m.value ? ` with ${m.selectedOptions[0].textContent}` : ' with your default model'}${navigator.platform.startsWith('Win') ? '; your prompt goes on the clipboard' : ', starting on your prompt'}.`,
      app: 'Opens the Claude app in the folder with your default model; your prompt goes on the clipboard.',
    }[t.value]
  ...
  async function start() {
    ...
    onRemember()
    const ok = await village.startSession(f, box.querySelector('[data-f="prompt"]').value.trim(), { target, model })
    if (ok) close()
  }

  box.addEventListener('change', (e) => {
    if (e.target.dataset.f === 'target' || e.target.dataset.f === 'model') explain(e.target.dataset.f)
