...
test('new sessions open the folder and carry the prompt through Cursor\'s prompt link', async (t) => {
  ...
  assert.deepEqual(r.urls, ['cursor://file/work/my%20app/?windowId=_blank', 'cursor://anysphere.cursor-deeplink/prompt?text=fix+%26+test'])
  assert.equal(r.promptPassed, true)
  assert.equal((await a.newSession('/work/app', { target: 'cursor' })).url, 'cursor://file/work/app/?windowId=_blank')
  const long = await a.newSession('/work/app', { target: 'cursor', prompt: 'x'.repeat(5000) })
  assert.equal(long.url, 'cursor://file/work/app/?windowId=_blank')
